...
function initSchema(db: Database.Database) {
  ...
  if (count.c === 0) {
    ...
    const sampleDefects: Array<[string, string, string, string, string, string, string, string, string, number, string, string]> = [
      ...
      ['DEF-2026-0120', 'ICPR Sertifikat upload timeout (sprint 22)', 'ICPR (Indonesia Credit Penjaminan Repository) Sertifikat Penjaminan upload to api.icpr.ojk.go.id timed out (HTTP 504); 1 cert dropped from sprint 22 batch — OJK 7-day SLA at risk', 'Major', 'High', 'RESOLVED', 'Integration Timeout', 'Penjaminan / ICPR', 'Backend Team', 0.93, '2026-03-30T09:14:00', '2026-04-02T17:00:00'],
      ['DEF-2026-0122', 'ICPR Sertifikat upload timeout (sprint 23 #1)', 'ICPR upload timeout — 1 cert dropped from sprint 23 batch (recurring from DEF-0120); raised HTTP timeout to 60s, still insufficient for >2MB cert PDFs', 'Major', 'High', 'RESOLVED', 'Integration Timeout', 'Penjaminan / ICPR', 'Backend Team', 0.94, '2026-04-04T09:08:00', '2026-04-05T16:30:00'],
      ['DEF-2026-0125', 'ICPR Sertifikat upload timeout (sprint 23 #2)', 'ICPR upload timeout — 2 certs dropped this run (escalating); ICPR upstream slow during 09:00-11:00 WIB, no retry logic in client', 'Major', 'High', 'RESOLVED', 'Integration Timeout', 'Penjaminan / ICPR', 'Backend Team', 0.95, '2026-04-08T09:22:00', '2026-04-09T15:45:00'],
      ['DEF-2026-0130', 'ICPR Sertifikat upload timeout (sprint 24)', '4th occurrence — 6 certs dropped (escalation 1→1→2→6); pattern systemic; needs retry+circuit breaker+DLQ before OJK 7-day SLA breach on 2026-04-22', 'Critical', 'High', 'OPEN', 'Integration Timeout', 'Penjaminan / ICPR', 'Backend Team', 0.97, '2026-04-13T09:14:00', '2026-04-13T09:14:00'],

      // Recurring pattern: IJP calculation drift (2 occurrences)
      ['DEF-2026-0128', 'IJP rate calculation drift between Maker and Approver', 'IJP rate Maker calculated 1.25% but Approver saw 1.50% — using stale PKS rate cached before mid-day update', 'Major', 'High', 'IN_PROGRESS', 'Cache Invalidation', 'Penjaminan / IJP Calculation', 'Backend Team', 0.88, '2026-04-11T11:30:00', '2026-04-12T14:00:00'],
      ['DEF-2026-0133', 'IJP rate type drift (number vs string)', 'IJP rate for cabang Bandung saved as 1.5 (number) but stored as "1.5" (string) in MySQL; type coercion lost during JSON marshalling', 'Minor', 'Medium', 'RESOLVED', 'Type Coercion', 'Master Data / IJP Rate', 'Backend Team', 0.82, '2026-04-13T10:45:00', '2026-04-13T16:00:00'],

      // Data integrity: NULL NIK + NPWP format
      ['DEF-2026-0129', '3 rows with NULL nik in cif_registrations', 'Upstream form allows save-as-draft without NIK; staging table missing NOT NULL constraint', 'Critical', 'High', 'OPEN', 'Data Integrity', 'Registrasi CIF', 'Frontend Team', 0.99, '2026-04-12T08:30:00', '2026-04-12T08:30:00'],
      ['DEF-2026-0127', 'NPWP accepts hyphen format (should be dotted DJP)', 'NPWP validation accepts XX-XXX-XXX-X-XXX-XXX (legacy hyphen) instead of dotted DJP format; legacy data carries hyphens', 'Minor', 'Low', 'OPEN', 'Validation Rule', 'Registrasi CIF', 'Backend Team', 0.86, '2026-04-10T14:20:00', '2026-04-10T14:20:00'],
      ['DEF-2026-0124', 'NIK validation accepts 15-digit value', 'NIK regex bug — used \\d{15,16} instead of \\d{16}', 'Major', 'High', 'RESOLVED', 'Validation Rule', 'Registrasi CIF', 'Backend Team', 0.95, '2026-04-06T10:15:00', '2026-04-07T17:00:00'],

      // Klaim & Subrogasi
      ['DEF-2026-0131', 'Klaim payout rejected: invalid BTN account format', 'Klaim payout to partner bank rejected — BTN savings uses 13-digit account but validator only accepts 10-digit', 'Major', 'High', 'OPEN', 'Validation Rule', 'Klaim / Bank Payment', 'Integration Team', 0.91, '2026-04-13T11:50:00', '2026-04-13T11:50:00'],
      ['DEF-2026-0132', 'Subrogasi recovery not posted to Oracle GL', 'Subrogasi recovery (Rp 250jt) not reflected in Oracle GL journal — integration job ran but transaction was rolled back silently', 'Major', 'High', 'IN_PROGRESS', 'Silent Failure', 'Subrogasi / Oracle GL', 'Backend Team', 0.89, '2026-04-13T13:10:00', '2026-04-13T13:10:00'],

      // Master data + workflow + housekeeping
      ['DEF-2026-0123', 'New PKS partner code not in dropdown', 'BTN cabang Bandung PKS code not visible in dropdown — master sync job missed nightly window', 'Minor', 'Medium', 'RESOLVED', 'Master Sync', 'Master Data / PKS LOV', 'Data Team', 0.84, '2026-04-05T08:45:00', '2026-04-06T12:00:00'],
      ['DEF-2026-0126', 'Checker approval email not delivered', 'Maker did not receive Checker approval notification — SMTP relay misconfig after firewall change', 'Minor', 'Medium', 'RESOLVED', 'Configuration Error', 'Workflow / Approval', 'Platform Team', 0.85, '2026-04-09T15:00:00', '2026-04-10T11:30:00'],
